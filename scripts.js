...
function resultTrendSvg_(trend){const pts=(trend||[]).slice(0,20);if(!pts.length)return '';const W=760,H=310,L=58,R=24,T=54,B=62,plotW=W-L-R,plotH=H-T-B;const x=i=>L+(pts.length===1?plotW/2:(i*(plotW/(pts.length-1))));const y=v=>T+(100-Math.max(0,Math.min(100,Number(v)||0)))*plotH/100;const line=(field)=>pts.map((p,i)=>`${x(i).toFixed(1)},${y(p[field]).toFixed(1)}`).join(' ');const labels=pts.map((p,i)=>{const label=String(p.exam||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').slice(0,18);const xx=x(i);return `<text x="${xx}" y="${H-30}" text-anchor="middle" font-size="10" fill="#64748b" transform="rotate(-25 ${xx} ${H-30})">${label}</text>`}).join('');const grid=[0,25,50,75,100].map(v=>{const yy=y(v);return `<line x1="${L}" x2="${W-R}" y1="${yy}" y2="${yy}" stroke="#e5e7eb"/><text x="${L-8}" y="${yy+4}" text-anchor="end" font-size="10" fill="#64748b">${v}%</text>`}).join('');const dots=(field,color)=>pts.map((p,i)=>`<circle cx="${x(i)}" cy="${y(p[field])}" r="4" fill="${color}"/>`).join('');const scope=String(pts[0]?.topperScope||'Entire ERP'),cat=String(pts[0]?.topperCategory||pts[0]?.category||''),cls=String(pts[0]?.topperClass||pts[0]?.className||'');const benchmark=[scope,cat,cls].filter(Boolean).join(' • ');const topNames=[...new Set(pts.map(p=>String(p.topperName||'').trim()).filter(Boolean))];const topNote=topNames.length?`<div style="font-size:11px;color:#64748b;margin:1px 0 7px">Benchmark: ${escapeHtml(topNames.slice(0,2).join(', '))}${topNames.length>2?' and others':''} • ${escapeHtml(benchmark)}</div>`:'';return `<div style="margin:16px 0 18px;padding:14px 16px;border:1px solid #dbe3ee;border-radius:12px;background:#fff"><div style="font-weight:700;color:#0b2f63;margin-bottom:2px">Performance Trend — Student vs Class Topper</div>${topNote}<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="100%" role="img" aria-label="Student performance compared with entire ERP class topper">${grid}<polyline fill="none" stroke="#0b2f63" stroke-width="3" points="${line('studentPct')}"/><polyline fill="none" stroke="#94a3b8" stroke-width="3" stroke-dasharray="6 5" points="${line('topperPct')}"/>${dots('studentPct','#0b2f63')}${dots('topperPct','#94a3b8')}${labels}<rect x="${L}" y="24" width="12" height="3" fill="#0b2f63"/><text x="${L+18}" y="29" font-size="10" fill="#334155">Student</text><rect x="${L+90}" y="24" width="12" height="3" fill="#94a3b8"/><text x="${L+108}" y="29" font-size="10" fill="#334155">Class Topper — Entire ERP</text></svg></div>`;}
...
function resultIndividualStudentBlockHtml_(details,trend,mode='screen'){const categories=resultUniqueValues_(details,'Category_Name'),classes=resultUniqueValues_(details,'Class_Name'),batches=resultUniqueValues_(details,'Batch_Code'),uin=String(details[0]?.UIN||''),name=String(details[0]?.Student_Name||''),father=String(details[0]?.Father_Name||''),graph=(trend||[]).length?resultTrendSvg_(trend):'';const row=(label,val)=>`<div class="student-detail-row"><span class="student-detail-label">${escapeHtml(label)}</span><span class="student-detail-value">${escapeHtml(val||'—')}</span></div>`;return `<div class="student-left-panel"><div class="student-panel-title">Student Details</div>${row('UIN',uin)}${row('Student Name',name)}${row("Father's Name",father)}${row('Category',categories.join(', '))}${row('Class',classes.join(', '))}${batches.length?row('Batch',batches.join(', ')):''}${graph?`<div class="student-trend-wrap">${graph}</div>`:''}</div>`;}
function resultIndividualExamTablesHtml_(details){const groups=resultStreamOrder_(details);const blocks=[];for(const stream of ['JEE','NEET','OTHER']){const rows=groups[stream]||[];if(!rows.length)continue;const subjects=resultSubjectsForGroup_(rows,stream);const label=stream==='OTHER'?'Subject-wise Results':stream+' Results';const head=`<th>Exam</th><th>Date</th>${subjects.map(s=>`<th>${escapeHtml(s)}<br><span class="subject-head-sub">Obt / Max</span></th>`).join('')}<th>Total</th><th>%</th><th>Rank</th>`;const body=rows.map(r=>`<tr><td class="exam-cell"><b>${escapeHtml(r.Exam_Name||'Exam')}</b></td><td>${escapeHtml(String(r.Exam_Date||'').slice(0,10))}</td>${subjects.map(s=>`<td class="marks-cell">${escapeHtml(resultSubjectCellText_(r,s))}</td>`).join('')}<td>${escapeHtml(String(r.Total_Obtained_Marks??r.Total_Marks??''))} / ${escapeHtml(String(r.Total_Max_Marks??r.Max_Total_Marks??''))}</td><td><b>${escapeHtml(resultPercentageText_(r.Percentage))}</b></td><td>${escapeHtml(String(r.Rank||'—'))}</td></tr>`).join('');blocks.push(`<div class="individual-results-group"><div class="individual-group-title">${escapeHtml(label)}</div><div class="table-wrap"><table class="data-table individual-results-table"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div></div>`);}return blocks.join('')||'<div class="muted">No subject-wise result records available.</div>';}
